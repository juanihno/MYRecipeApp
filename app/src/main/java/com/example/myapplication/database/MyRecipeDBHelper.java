...
public class
MyRecipeDBHelper extends SQLiteOpenHelper {
    ...
    private static final String COL_USER_ID_RECIPE = "USERID";
    ...
    private static final String GET_RANDOM_RECIPE = "SELECT * FROM " + TABLE_RECIPE + " ORDER BY RANDOM() LIMIT 1" ;
    ...
    public boolean checkUserName (String username){
        SQLiteDatabase db=this.getWritableDatabase();
        @SuppressLint("Recycle") Cursor cursor=db.rawQuery("SELECT * FROM " + TABLE_USER + " WHERE " + COL_USER_NAME + "= ?",new String[]{username});
    ...
    public boolean checkUserPassword (String username, String password){
        SQLiteDatabase db=this.getWritableDatabase();
        @SuppressLint("Recycle") Cursor cursor=db.rawQuery("SELECT * FROM " + TABLE_USER + " WHERE " + COL_USER_NAME + "= ?" + " AND " + COL_USER_PASSWORD+"=?",new String[]{username, password});
    ...
    /*public Long getUserId(String username, String password) {
        ...
        //Cursor cursor = db.rawQuery(" SELECT  "+ COL_USER_NAME +" FROM "+ TABLE_USER + " WHERE " + COL_USER_NAME + "= ?" + " AND " + COL_USER_PASSWORD+"=?",new String[]{username, password});
        Long userId = cursor.getLong(cursor.getColumnIndex(COL_USER_ID));
        return userId;}*/
     ...
     *              otherwise -1
     */
    public Long insertRecipe(String name, String description, Integer difficulty,byte[] image, Long userId ){
    ...
    /**
     * @return  A cursor of all recipes in the table called monster.
     */
    ...
    private Cursor getAllRecipesbyUserId(Long userId) {
        SQLiteDatabase db = this.getReadableDatabase();
        return db.rawQuery(GET_RECIPES_BY_USER, new String[]{userId.toString()});
    }
     ...
     */
    public boolean updateRecipe(Long id, String name, String description, Integer difficulty,byte[] image) {
        SQLiteDatabase db = this.getWritableDatabase();
        ...
        db.update(TABLE_RECIPE, contentValues, "ID = ?", new String[]{id.toString()});
        db.close();
        return true; //numOfRowsUpdated == 1; //if your query is going to update more than 1 record (this is not the case) then the condition will be numRowsUpdated > 0
    }
    ...
    public List<Recipe> getRecipes() {
        ...
        if(cursor.getCount() > 0) {
            ...
            while (cursor.moveToNext()) {
                ...
                byte[] imageFileName = cursor.getBlob(4);
                Integer votes = cursor.getInt(5);
                Integer stars = cursor.getInt(6);
                Long userId = cursor.getLong(7);


                recipe = new Recipe(id, name, description, difficulty, imageFileName, votes, stars, userId);
                recipes.add(recipe);
            }
        }
        cursor.close();
    ...
    public List<Recipe> getRecipesByUserId(Long userId) {
        List<Recipe> recipes = new ArrayList<>();
        Cursor cursor = getAllRecipesbyUserId(userId);
        ...
        SQLiteDatabase db = this.getReadableDatabase();
        Recipe recipe = null;
        Cursor cursor = db.rawQuery(GET_RECIPES_BY_USER, new String[]{id.toString()});
    ...
    public Recipe getRandomRecipe() {
        SQLiteDatabase db = this.getReadableDatabase();
        Recipe recipe = null;
        Cursor cursor = db.rawQuery(GET_RANDOM_RECIPE, null);
    ...
    public void insert(byte[] bytes)
    {
        ...
        cv.put(COL_RECIPE_IMAGE,bytes);
        getWritableDatabase().insert(TABLE_RECIPE,COL_RECIPE_IMAGE,cv);
